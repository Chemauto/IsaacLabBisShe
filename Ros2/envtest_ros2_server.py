...
- 转成 Socket server 已支持的文本协议
- 写入 `envtest_model_use_player.py` 默认读取的控制文件

这个脚本刻意复用 `Socket/envtest_socket_server.py` 的解析和写文件逻辑，
避免 UDP 和 ROS2 两套控制入口出现行为差异。
"""
...
from Socket.envtest_socket_server import OutputPaths, apply_message  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Receive EnvTest control commands through ROS2 topics.")
...
class EnvTestRos2ControlNode(Node):
    def __init__(self, output_paths: OutputPaths, args: argparse.Namespace):
        super().__init__("envtest_ros2_control_server")
