...
# 输出的
OUTPUT_SCENE_PATH = SCRIPT_DIR.parent / "unitree_robots" / ROBOT / "scene_terrain.xml"
```
3. 运行
```bash
...
```
程序会把地形场景文件输出到 `Mujoco/unitree_robots/go2/scene_terrain.xml`。
如果要生成当前自定义的 mine 地形示例，可以运行：
```bash
cd Mujoco/terrain_tool
python3 ./mine_terrain_generator.py
```
它会输出到 `Mujoco/unitree_robots/go2/mine_scene_terrain2.xml`。
如果使用的是当前仓库里的 python 仿真器，则修改 `Mujoco/simulate_python/config.py`：
```python
...
```
之后运行 `Mujoco/simulate_python` 下面的 MuJoCo 仿真器，就可以看到生成的地形。
