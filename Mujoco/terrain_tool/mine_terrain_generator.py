...
class TerrainGenerator:
    ...
    def AddPerlinHeighField(
        ...
        output_hfield_path = ROBOT_DIR / output_hfield_image
        cv2.imwrite(str(output_hfield_path), terrain_image)
    ...
    def AddHeighFieldFromImage(
        ...
        input_img_path = Path(input_img)
        if not input_img_path.is_absolute():
            input_img_path = SCRIPT_DIR / input_img_path

        input_image = cv2.imread(str(input_img_path))
...
if __name__ == "__main__":
    ...
    # # Slope
    # tg.AddStairs(init_pos=[2.0, 2.0, 0.0], yaw=0.0)

    # # Stairs
    # tg.AddStairs(init_pos=[1.0, 4.0, 0.0], yaw=0.0)

    # # Suspend stairs
    # tg.AddStairs(init_pos=[1.0, 6.0, 0.0], yaw=0.0)

    # # Rough ground
    # tg.AddStairs(init_pos=[-2.5, 5.0, 0.0], yaw=0.0)

    # # Perlin heigh field
    # tg.AddStairs(init_pos=[-1.5, 4.0, 0.0], yaw=0.0)
