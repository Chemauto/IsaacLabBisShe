...
class EventCfg:
    ...
    # startup，这个是物理参数随机化、摩擦、弹性
    physics_material = EventTerm(
        func=mdp.randomize_rigid_body_material,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*"),
            "static_friction_range": (0.3, 1.2),#动摩擦力
            "dynamic_friction_range": (0.3, 1.0),#静摩擦力
            "restitution_range": (0.0, 0.30),#弹性系数
            "num_buckets": 64,
        },
    )
    #base增加和减少重量，并且重新计算惯性
    ...
    scale_bodyothers_mass_ = EventTerm(
        func=mdp.randomize_rigid_body_mass,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["^(?!.*base).*"]),
    ...
    base_external_force_torque = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
            "force_range": (-6.0, 6.0),#暂定给的
            "torque_range": (-2.0, 2.0),#暂定给的
        },
    )
    ...
    reset_robot_joints = EventTerm(
        ...
        mode="reset",
        params={
            "position_range": (0.8, 1.25),
            "velocity_range": (-0.4, 0.4)#原本都是0,
        },
    )
