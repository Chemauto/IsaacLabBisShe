...
class AdvancedPose2dCommand(UniformPose2dCommand):
    """论文风格 episode process：

    - 以机器人初始位置为中心，极坐标采样半径 [1, 5]m 目标。
    - 过滤无效目标（坑底/高障碍），无效目标持续重采样。
    """
    ...
    def __init__(self, cfg: AdvancedPose2dCommandCfg, env):
        ...
        if self.cfg.use_valid_target_patches:
            try:
                ...
                raise RuntimeError("[ManualTest] Terrain asset unavailable for target validity filtering.") from exc
            if self.valid_targets is None:
                raise RuntimeError(
                    f"[ManualTest] No flat patches found for key '{self.cfg.target_patch_name}'."
                )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if unresolved.numel() > 0 and candidate_targets is not None:
            local_centers = centers[unresolved]
            local_candidates = candidate_targets[unresolved]
        ...
        if unresolved.numel() > 0:
            ...
            if self.cfg.fallback_to_polar_sampling:
                ...
                if (
                    ...
                    logger.warning(
                        "[ManualTest] Target patch filtering fallback for %d envs at step %d.",
                        missing_count,
                ...
                raise RuntimeError(
                    "[ManualTest] Failed to sample valid targets for "
